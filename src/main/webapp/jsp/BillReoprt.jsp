...
<%  double mobile = (double) request.getAttribute("MobileNo");%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
...
<form action="/ZOHO_HOSPITAL/final"> <%
         ...
         try{
        	Connection con = Conn.getCon();
 			...
 			if(rs1.next()==false){
 				...
 				registrationCost=0;
 			} else{ %><h6>Report Of <%= rs1.getString(2)%>  having patient Id "<%= rs1.getInt(1)%>"</h6><%
	 ...
	 ResultSet rs2 = stmt1.executeQuery(qry2);
 			...
 			if(rs2.next()==false){
 				out.println("No Reports Is Available!...");
 				registrationCost=0;
 			...
 			if(rs3.next()==false){
 				out.println("No Medicine Report Is Available!...");
 				registrationCost=0;
 			} else{%>
 				<table>
 			...
 			out.println("Total Test Expenses With Us Till Now: "+ testCost+"/-");%> </br> <% 
 			out.println("Total Medicine Expenses With Us Till Now: "+medicineCost+"/-");%> </br> <% 
 			out.println("Total Amount Paid : "+total+"/-");
